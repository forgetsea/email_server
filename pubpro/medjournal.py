...
file_handler = logging.FileHandler('pubmed_log.log')
...
def output_to_excel(search_term, info):
    ...
    if info_list:
        ...
        df.to_excel(os.path.join(output_dir, f"{search_term}_output.xlsx"), index=False)
    else:
...
def medpro_search(keywordA, keywordB='', input_date=''):
    ...
    logger.debug(f"Processing article {article_count}, stored email {email_count} ")
    output_to_excel(filename, info_list)
    output_to_excel(f"{filename}_CN", cn_list)
